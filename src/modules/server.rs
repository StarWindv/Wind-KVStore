...
async fn index(req: HttpRequest) -> impl Responder {
    utils::server_info(
        utils::get_client_ip(&req).as_str(),
        req.method().as_str(),
        req.path(),
    );
...
async fn open_db(
    ...
    utils::server_info(
        utils::get_client_ip(&http_req).as_str(),
    ...
    utils::format_header(&http_req, PRINT_HEADER.clone());
...
async fn close_db(sessions: Data<SessionManager>, http_req: HttpRequest) -> impl Responder {
    ...
    let session_id = utils::get_session_from_header(&http_req);
    utils::format_session_id(&session_id);
    utils::format_header(&http_req, PRINT_HEADER.clone());
